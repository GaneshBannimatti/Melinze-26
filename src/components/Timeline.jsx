...
const day2Events = [
  ...
  { time: "12:30 PM", title: "Winner Announcement & Valedictory & Prizes", icon: "🎊" },
];
...
export default function Timeline() {
  ...
  return (
    <section id="timeline" className="py-16 sm:py-20 px-4 text-white">
      ...
      <div className="flex justify-center mb-10">
        <div className="flex gap-2 p-1 border border-cyan-400 rounded-xl">
          {["March 12th ", "March 13th"].map((label, i) => (
            <button
              key={label}
              onClick={() => setDay(i + 1)}
              className={`px-4 py-1.5 text-xs sm:text-sm rounded-lg font-semibold transition ${
                day === i + 1
                  ? "bg-white text-black"
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-5">
        {events.map((event, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="bg-neutral-900/90 border border-white/10 rounded-xl px-5 py-4
                       hover:border-cyan-400 hover:shadow-[0_0_18px_rgba(34,211,238,0.25)]
                       transition"
          >
